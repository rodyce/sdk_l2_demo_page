...
const App = () => {
  ...
  async function onSubmitTransfer(event: React.FormEvent<HTMLFormElement>) {
    ...
    const fee = await l2Provider!.getTransferFee(toAddress, workingToken);
  ...
  async function refreshBalance() {
    const balance = await l2Wallet!.getTokenBalance(workingToken);
  ...
  function handleWorkingToken(event: React.FormEvent<HTMLSelectElement>) {
    const { name, value } = event.currentTarget;
    console.log(`name: ${name}, value: ${value}`);
    setWorkingToken(value);
  }
  ...
  async function onSubmitDeposit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const deposit =
      workingToken === "ETH"
        ? Deposit.createDeposit({ toAddress, amount, fee })
        : Deposit.createTokenDeposit({
            toAddress,
            amount,
            fee,
            tokenSymbol: workingToken,
  ...
  return (
    <>
      ...
      <form onSubmit={onSubmitTransfer}>
        ...
        Result:
        <br />
        <textarea readOnly={true} value={confirmed} rows={10} cols={50} />
      </form>
      <br />
      <br />
      <br />
      ...
      <form onSubmit={onSubmitDeposit}>
        ...
        Result:
        <br />
        <textarea readOnly={true} value={depositResult} rows={10} cols={50} />
      </form>
    </>
  );
};
